...
- 当前版本会优先从 `Resources` 加载战斗 BGM。
- 如果 `Resources` 路径失败，会继续尝试 `StreamingAssets/Audio/BGM/hyoshi_action_track_2.ogg`。
- 只有这两条都失败时，才会回落到程序生成的临时战斗 BGM。
- 如果你改过音频文件，记得重新打包后再复制到另一台电脑。
